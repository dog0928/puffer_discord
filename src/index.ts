...
client.once(Events.ClientReady, async (readyClient) => {
	...
	for (const [name] of (client as ChatInputCommandInteraction).commands)
		console.log(`- ${name}`);
	...
	console.log(
		`=== Guild Application Commands (Discord side) : ${guild.name} ===`,
	);
	...
	for (const [id, cmd] of globalCommands)
		console.log(`- ${cmd.name} (id=${id})`);
...
for (const folder of commandFolders) {
	...
	for (const file of commandFiles) {
		...
		if (command && "data" in command && "execute" in command) {
			(client as ChatInputCommandInteraction).commands.set(
				command.data.name,
				command,
			);
		} else {
...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	const command = (
		interaction.client as ChatInputCommandInteraction
