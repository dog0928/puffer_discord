export class Puffer {
	async get() {
		...
		const response = await fetch(`http://${process.env.PUFFER_SERVER_URL}:8080/oauth2/token`, {
			method: "POST",
			headers: {
	...
	async status() {
		...
		const response = await fetch(`http://${process.env.PUFFER_SERVER_URL}:8080/api/servers/${process.env.PUFFER_SERVER_ID}/status`, {
			method: "GET",
			headers: {
				"Content-Type": "application/x-www-form-urlencoded",
	...
	async start() {
		...
		const response = await fetch(`http://${process.env.PUFFER_SERVER_URL}:8080/api/servers/${process.env.PUFFER_SERVER_ID}/start`, {
			method: "POST",
			headers: {
				"Content-Type": "application/x-www-form-urlencoded",
				"Authorization": `Bearer ${getToken.token}`,
	...
	async stop() {
		...
		const response = await fetch(`http://${process.env.PUFFER_SERVER_URL}:8080/api/servers/${process.env.PUFFER_SERVER_ID}/stop`, {
			method: "POST",
			headers: {
				"Content-Type": "application/x-www-form-urlencoded",
				"Authorization": `Bearer ${getToken.token}`,
